...
def run_inference_on_gpu(rank, model, processor, vqvae_model, sample_prompts, output_path, cfg_scale, image_token_count, img_nums_per_prompt):
    ...
    for index, metadata in tqdm(enumerate(sample_prompts[start_index:end_index]), desc=f'pid-[{str(rank)}]'):
        outpath = os.path.join(output_path, f"{start_index + index:0>5}")
        ...
        for idx in range(img_nums_per_prompt):
            ...
            try:
                ...
                generated_ids = model.mar_t2i_generate(inputs, null_inputs, vqvae_model, eot_token=tokenizer.eos_token_id, top_k=1000, top_p=1.0, cfg=cfg_scale, generate_start_id=generate_start_id, generate_end_id=generate_end_id, eol_id=eol_id, tgt_h = resolution, tgt_w=resolution, ds = vqvae_downscale).squeeze(0)
                ...
                Image.fromarray(sample).save(os.path.join(sample_path, f"{sample_count:05}.png"))
                sample_count += 1
...
if __name__ == '__main__':
    ...
    torch.multiprocessing.spawn(run_inference_on_gpu, args=(model, processor, vqvae_model, sample_prompts, output_path, cfg_scale, image_token_count, img_nums_per_prompt), nprocs=world_size)
