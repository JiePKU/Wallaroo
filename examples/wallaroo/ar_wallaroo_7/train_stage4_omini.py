...
from wallaroo.datasets.prompting_utils import UniversalPrompting, create_attention_mask_predict_next, create_attention_mask_for_mmu, create_attention_mask_for_mmu_vit_v1
from wallaroo.datasets import MARImage2ImageDataset, MARText2ImageDataset,  LLAVAMmuDataset
...
from wallaroo.utils.logging import set_verbosity_info, set_verbosity_error, is_chinese, is_english, check_string
from wallaroo.utils.vis_utils import visualize_mmu_predictions, visualize_mar_t2i_predictions_1d, random_masking
...
def process_edit_batch(edit_batch, w_pixel_vae, device):

    edit_semantic_values, edit_pixel_values, edited_pixel_values = edit_batch['semantic_values'], edit_batch['pixel_values'], edit_batch["images"]
    edit_input_ids, edit_attention_mask, edit_labels = edit_batch['input_ids'].to(device, non_blocking=True), edit_batch['attention_mask'].to(device, non_blocking=True), edit_batch['labels'].to(device, non_blocking=True)
    edit_pixel_grid_thw = edit_batch['pixel_grid_thw'].to(device, non_blocking=True)
    edit_semantic_values = [edit_semantic_value.to(device, non_blocking=True) for edit_semantic_value in edit_semantic_values]
    edit_image_grid_thw = [image_grid_thw.to(device, non_blocking=True) for image_grid_thw in edit_batch['image_grid_thw']]

    if w_pixel_vae:

        edit_pixel_values = [edit_pixel_value for edit_pixel_value in edit_pixel_values]
        edit_pixel_features = []

        for edit_pixel_value in edit_pixel_values:
            with torch.no_grad():
                with torch.cuda.amp.autocast(enabled=False):

                    z_q, _, [_, z, raw_indice] = pixel_vae_model.encode(edit_pixel_value.to(accelerator.device, non_blocking=True).unsqueeze(0))
                    
                    z = z.reshape(z.shape[0], -1, z.shape[3]).to(device).to(weight_dtype) # unquanted vector  z shape b h w c

                    ## mask 60% informative token
                    masked_z, ids_restore = random_masking(z, mask_ratio=0.6)
                    mask_tokens = mask_token.repeat(masked_z.shape[0], ids_restore.shape[1] - masked_z.shape[1], 1).to(device).to(weight_dtype)
                    z = torch.cat([masked_z, mask_tokens], dim=1)  # no cls token
                    z = torch.gather(z, dim=1, index=ids_restore.unsqueeze(-1).repeat(1, 1, z.shape[2]))  # unshuffle

            vae_features = model.pixel_adapter(z)
            edit_pixel_features.append(vae_features.squeeze(0))

    else:       
        edit_pixel_features = None
        edit_pixel_grid_thw = None
    
    edit_text_token_labels, edit_image_token_labels = torch.clone(edit_labels), torch.clone(edit_labels)
    continue_vq_features = []

    for i, (edit_input_id, edit_label, edited_pixel_value) in enumerate(zip(edit_input_ids, edit_labels, edited_pixel_values)):
        with torch.no_grad():
            if  vqvae_config.type in ['LlamaGen_16x16',  'MOVQGAN_8x8']:
                z_q, _, [_, _, edit_indice] = vqvae_model.encode(edited_pixel_value.to(accelerator.device, non_blocking=True).unsqueeze(0))
                codebook_entry = vqvae_model.quantize.get_codebook_entry(edit_indice).unsqueeze(0).to(device).to(weight_dtype)
            else:
                print("Not support vqvae type")
                raise NotImplementedError
                
        continue_vq_feature = model.edit_input_adpater(codebook_entry)
        continue_vq_features.append(continue_vq_feature)

        generate_image_mask = (edit_input_id == image_generate_pad_id)
        generate_img_tokens = generate_image_mask.nonzero(as_tuple=True)[0].shape[0]
        if generate_img_tokens != edit_indice.shape[0]: 
            raise ValueError(
                            f"T2i image pad tokens and vqvae tokens do not match: tokens: {generate_img_tokens}, features {edit_indice.shape[0]}"
                        )

        edit_image_token_labels[i] = torch.full(edit_input_id.shape, -100).to(device).masked_scatter(generate_image_mask, edit_indice)
        edit_text_token_labels[i] = edit_label.masked_fill(generate_image_mask, -100)

    edit_attention_mask = edit_attention_mask.to(mask_dtype)

    return edit_semantic_values, edit_input_ids, edit_attention_mask, edit_image_token_labels, edit_text_token_labels, edit_pixel_features, continue_vq_features, edit_image_grid_thw, edit_pixel_grid_thw
...
def train(global_step=0):
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for batch, step, dataloader_idx in combined_dataloader:
            ...
            # *-------*-------*-------*-------*-------*-------*-------*-------*-------*-------*-------*
            # Build formatted sequences for i2i image edit
            # *-------*-------*-------*-------*-------*-------*-------*-------*-------*-------*-------*
            edit_semantic_values, edit_input_ids, edit_attention_mask, edit_image_token_labels, edit_text_token_labels, edit_pixel_features, edit_vq_features, edit_image_grid_thw, edit_pixel_grid_thw = process_edit_batch(edit_batch=batch["edit_flow"], w_pixel_vae=w_pixel_vae, device=accelerator.device)
            edit_mm_inputs = {  
                                'input_ids': edit_input_ids, 
                                'continue_vq_features': edit_vq_features, 
                                'attention_mask': edit_attention_mask, 
                                'image_token_labels': edit_image_token_labels, 
                                'text_token_labels': edit_text_token_labels,
                                'pixel_values': edit_semantic_values, 
                                'pixel_vae_values': edit_pixel_features,
                                'image_grid_thw': edit_image_grid_thw,
                            }  
            ...
            with accelerator.accumulate(model):
                ...
                mmu_loss, t2i_loss, t2i_text_token_loss, t2i_image_token_loss, edit_loss, edit_text_token_loss, edit_image_token_loss = model.forward_omni(t2i_inputs=t2i_mm_inputs, mmu_inputs=mmu_mm_inputs, edit_inputs=edit_mm_inputs)

                text_token_loss = t2i_text_token_loss + edit_text_token_loss

                # Gather the losses across all processes for logging (if we use distributed training)
                avg_mmu_loss = accelerator.gather(mmu_loss.repeat(config.training.batch_size_mmu)).mean()
                avg_t2i_image_token_loss = accelerator.gather(t2i_image_token_loss.repeat(config.training.batch_size_t2i)).mean()
                avg_edit_image_token_loss = accelerator.gather(edit_image_token_loss.repeat(config.training.batch_size_edit)).mean()
                ...
                avg_edit_loss = accelerator.gather(edit_loss.repeat(config.training.batch_size_edit)).mean()
                ...
                loss = config.training.t2i_coeff * t2i_loss + config.training.mmu_coeff * mmu_loss + config.training.edit_coeff * edit_loss
            ...
            if accelerator.sync_gradients:
                ...
                logs = {'loss': accelerator.gather(loss).mean().item(), 'mmu_loss': avg_mmu_loss.item(), 't2i_loss': avg_t2i_loss.item(), 'edit_loss': avg_edit_loss.item(), 'text_token_loss': avg_text_token_loss.item(), 't2i_image_token_loss': avg_t2i_image_token_loss.item(), 'edit_image_token_loss':avg_edit_image_token_loss.item()}
...
if __name__ == "__main__":
    ...
    total_batch_size_per_gpu = config.training.batch_size_t2i + config.training.batch_size_edit + config.training.batch_size_mmu
    ...
    total_batch_size = ((config.training.batch_size_t2i + config.training.batch_size_edit + config.training.batch_size_mmu)* accelerator.num_processes * config.training.gradient_accumulation_steps)
    ...
    assert config.dataset.preprocessing.t2i_resolution in [512], "wrong t2i resolution for multiple aspect resolution training"

    tokenizer.add_tokens(list(special_tokens) + row_col_indicator_512_lst)
    ...
    if w_pixel_vae:
        mask_token = nn.Parameter(torch.ones(1, 1, 8) * -1, requires_grad=False)
    ...
    # pixel_vae_config = config.model.pixel_vae_model
    

    if config.dataset.gen_type == "mar_t2i":
    ...
    if config.dataset.edit_type == 'mar_i2i':
        data_args={'seg_image_min_pixels': preproc_config.SEG_MIN_PIXELS, 'seg_image_max_pixels': preproc_config.SEG_MAX_PIXELS, 'seg_size_factor': preproc_config.SEG_IMAGE_FACTOR, 'max_ratio': preproc_config.MAX_RATIO}
        dataset = MARImage2ImageDataset(
            root=dataset_config.train_i2i_shards_path_or_url,
            resolution=preproc_config.i2i_resolution,
            processor=processor,
            data_args=data_args,
            pixel_pad_left = config.model.wallaroo.pixel_pad_left,
            w_pixel_vae = w_pixel_vae,
            uncond_p=preproc_config.uncond_p,
            vqvae_downscale=preproc_config.vqvae_downscale,
            t2i_generate_method=config.model.wallaroo.i2i_generate_method,
            aspect_ratio_type=preproc_config.aspect_ratio_type,
            hw_indicator=preproc_config.hw_indicator,
            max_seq_length=preproc_config.i2i_max_seq_length)
        
        train_dataloader_edit = DataLoader(dataset, batch_size=config.training.batch_size_edit,
                                          collate_fn=dataset.collate_fn,
                                          shuffle=False, num_workers=dataset_config.num_workers)
    ...
    iterables = {
        ...
        "edit_flow": train_dataloader_edit
    }
    ...
    if w_pixel_vae:
        pixel_vae_model = pixel_vae_model.to(device=accelerator.device)
    else:
        pixel_vae_model = None
