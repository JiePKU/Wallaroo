...
class MARText2ImageDataset(torch.utils.data.IterableDataset): ## Mutiple Aspect Resoltion 
    def __init__(self, root, resolution, processor, max_seq_length, aspect_ratio_type, time_tokens=4, vqvae_downscale=16, hw_indicator=False, t2i_generate_method='line_break', **kwargs):
        self.root = root
        ...
        for file_name in self.sub_data_files:
            ...
            while True:
                try:
                    ...
                    assert data_source in ['blip3o_60k',  'Share4o', 'OpenGPT-4o'], print(data_source)
                    ...
                    if 'cn' in sel_prompt:
                        res = random.choice(chinese_generate_template).format(target_w=tgt_w, target_h=tgt_h, prompt=clean_prompt)
                    else:
                        res = random.choice(english_generate_template).format(target_w=tgt_w, target_h=tgt_h, prompt=clean_prompt)
                    ...
                    # print("gen sample length", input_ids.shape[0])
                    if input_ids.shape[0] > self.max_seq_length:
                        print("gen pass max length", input_ids.shape[0], data_source)
                        continue
