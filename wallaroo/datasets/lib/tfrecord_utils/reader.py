...
from . import example_pb2, iterator_utils
# from diffusion.data.datasets.lib.tfrecord_utils import example_pb2, iterator_utils
# import example_pb2, iterator_utils
...
def check_typename(typename, key, typename_mapping, inferred_typename):
...
def read_single_description(description, record, typename_mapping):
    ...
    sequence_example.ParseFromString(record)
    
    all_context_keys = list(sequence_example.context.feature.keys())
    all_sequence_keys = list(sequence_example.feature_lists.feature_list.keys())
    all_keys = list(set(all_context_keys) | set(all_sequence_keys))
    ...
    for key, typename in description.items():
        if key in all_context_keys:
            # NOTE: We assume that each key in the example has only one field
            # (either "bytes_list", "float_list", or "int64_list")!
            # field = example.features.feature[key].ListFields()[0]
            field = sequence_example.context.feature[key].ListFields()[0]
            inferred_typename, value = field[0].name, field[1].value
            check_typename(typename, key, typename_mapping, inferred_typename)
            ...
            features[key] = value
        elif key in all_sequence_keys:
            feature_list = sequence_example.feature_lists.feature_list[key]
            for feature in feature_list.feature:
                field = feature.ListFields()[0]
                inferred_typename, value = field[0].name, field[1].value
                check_typename(typename, key, typename_mapping, inferred_typename)
                if typename == "byte" or typename == "bytes":
                    value = np.frombuffer(value[0], dtype=np.uint8)
                elif typename == "float":
                    value = np.array(value, dtype=np.float32)
                elif typename == 'float16':
...
if __name__ == "__main__":
    data_path = '/mnt/dolphinfs/hdd_pool/docker/user/hadoop-vision-data/midjourney-messages/upscaled_prompts_df_flan_t5_xxl_result/part0_flan_t5_xxl_000001.tfrecord'
    start_offset, end_offset = 0, None
    description = {'state_shape': 'byte', 'state_dict': 'byte', 'meta': 'byte'}
    features = read_single_record_with_spec_index(data_path, start_offset, end_offset, description)
    print(features)
    from IPython import embed 
    embed()

    # data_path = '/mnt/dolphinfs/hdd_pool/docker/user/hadoop-mtcv/mahanghang/ep_data/split_00000.tfrecord'
    # start_offset, end_offset = 0, None
    # description = {'str_frames': 'bytes', 'title': 'byte', 'content': 'byte', 'ocr': 'byte', 'asr': 'byte'}
    # features = read_single_record_with_spec_index(data_path, start_offset, end_offset, description)
    # print(features)
    # from IPython import embed 
    # embed()
