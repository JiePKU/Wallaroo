...
from ..tfrecord_utils import common
from ..tfrecord_utils import tfrecord_loader
from ..tfrecord_utils.tools import create_index
...
class TFRecordReader(BaseIOReader):
    @classmethod
    def get_index(cls, data_file, description, index_file: str = "") -> List:
    ...
    def scan_or_create_index(cls, data_file, index_file: str = "") -> (str, int):
        if index_file == "":
            tmp_index_file = os.path.splitext(data_file)[0] + '.tfindex'
            if common.create_local_file_spinlock(tmp_index_file):
                # 不是rank0或者文件存在, 返回false
    ...
    @classmethod
    def convert_to_tfrecord(cls, data_file, description):
        return data_file
