...
from .dataset_mem import DatasetMem
...
class ChunkFileDataLoader:
    ...
    shuffle: bool
        should shuffle.
    """
    def __init__(self, data_config, data_path_list, desc_path, use_cuda, training, shuffle, dataset_cls=None):
        cfg = get_cfg()
        self.cfg = cfg
        if shuffle:
...
def build_dataloader(
    ...
    """
    build dp dataloader
    注意: DP dataloader 在 ddp 模式下使用, worker会得到重复的数据, 每个worker都过了一遍完整的数据
    ...
    """
    return ChunkFileDataLoader(data_config, data_path_list, desc_path, use_cuda, training, shuffle, dataset_cls)
