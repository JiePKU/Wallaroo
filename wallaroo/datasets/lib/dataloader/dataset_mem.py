...
from ..tfrecord_utils import read_single_record_with_spec_index
...
class DatasetMem(torch.utils.data.Dataset, BaseDataset, ABC):
    def __init__(self, cfg, data_file, desc_dict, training=True):
        if cfg is None:
            cfg = get_cfg()
        super().__init__(cfg, data_file, desc_dict, training)
