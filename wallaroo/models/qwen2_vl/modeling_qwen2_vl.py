...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def get_rope_index(
        ...
        pixel_start_token_id = self.config.pixel_start_token_id
        ...
        if input_ids is not None and (image_grid_thw is not None or video_grid_thw is not None):
            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                if not self.config.w_pixel_vae:
                    vision_start_indices = torch.argwhere(input_ids == vision_start_token_id).squeeze(1)
                else:
                    vision_start_indices = torch.argwhere((input_ids == vision_start_token_id) | (input_ids == pixel_start_token_id)).squeeze(1)
    ...
    def forward(
        ...
        pixel_vae_values: Optional[torch.Tensor] = None,
        pixel_values_videos: Optional[torch.FloatTensor] = None,
        image_grid_thw: Optional[torch.LongTensor] = None,
        pixel_grid_thw: Optional[torch.LongTensor] = None,
        video_grid_thw: Optional[torch.LongTensor] = None,
        rope_deltas: Optional[torch.LongTensor] = None,
        cache_position: Optional[torch.LongTensor] = None,
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                new_input_embeds = []
                for bs_idx, (input_embeds, input_id, pixel_value, image_grid) in enumerate(zip(inputs_embeds, input_ids, pixel_values, image_grid_thw)):
                    pixel_value = pixel_value.type(self.visual.get_dtype())
                    semantic_image_embed = self.visual(pixel_value, grid_thw=image_grid.unsqueeze(0))

                    semantic_start_index = (input_id == self.config.vision_start_token_id).nonzero(as_tuple=True)[0].item()
                    semantic_end_index = (input_id == self.config.vision_end_token_id).nonzero(as_tuple=True)[0].item()
                    ...
                    semantic_image_embed = semantic_image_embed.to(inputs_embeds.device, inputs_embeds.dtype)
                    input_embeds = input_embeds.masked_scatter(semantic_image_mask, semantic_image_embed)

                    if self.config.w_pixel_vae:
                        pixel_start_index = (input_id == self.config.pixel_start_token_id).nonzero(as_tuple=True)[0].item()
                        pixel_end_index = (input_id == self.config.pixel_end_token_id).nonzero(as_tuple=True)[0].item()
                        pixel_n_image_tokens = pixel_end_index - pixel_start_index - 1
                        ...
                        pixel_image_embed = pixel_image_embed.to(inputs_embeds.device, inputs_embeds.dtype)
                ...
                new_input_embeds = inputs_embeds
        ...
        if position_ids is None and (attention_mask is None or attention_mask.ndim == 2):
            ...
            if (
                ...
                if pixel_values is not None and self.config.w_pixel_vae:
                    if self.config.pixel_pad_left:
                        new_image_grid_thw = torch.cat([pixel_grid_thw.unsqueeze(1),  image_grid_thw.unsqueeze(1)], dim=1).view(-1, pixel_grid_thw.size(1))
                    else:
                        new_image_grid_thw = torch.cat([image_grid_thw.unsqueeze(1),  pixel_grid_thw.unsqueeze(1)], dim=1).view(-1, image_grid_thw.size(1))
                    
                    position_ids, rope_deltas = self.get_rope_index(input_ids, new_image_grid_thw, video_grid_thw, attention_mask)
