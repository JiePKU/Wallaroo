    ...
    def _prepare_4d_causal_attention_mask_with_cache_position(
        ...
        if attention_mask is not None and attention_mask.dim() == 4:
            ...
            if attention_mask is not None:
                ...
                # assert len(causal_mask[:, :, :, :mask_length].shape) == len(attention_mask[:, None, None, :].shape), f"({causal_mask.shape}, {mask_length}, {target_length}, {attention_mask.shape})"
                # print(f"({causal_mask.shape}, {mask_length}, {target_length}, {attention_mask.shape})")
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                for _ in range(image_nums + video_nums):
                    if image_token_id in input_tokens and remain_images > 0:
                        ed_image = input_tokens.index(vision_start_token_id, st) + 1
                    else:
    ...
    def forward(
        ...
        continue_vq_features: Optional[List] = None,
        image_grid_thw: Optional[torch.LongTensor] = None,
        video_grid_thw: Optional[torch.LongTensor] = None,
        rope_deltas: Optional[torch.LongTensor] = None,
        cache_position: Optional[torch.LongTensor] = None,
        ...
        if inputs_embeds is None:
            ...
            if continue_vq_features is not None: ## for image generation

                generate_mask = (input_ids == self.config.generate_img_pad_id).unsqueeze(2).expand(-1, -1, inputs_embeds.shape[2])

                length = len(continue_vq_features)

                for i in range(length):
                    inputs_embeds[i] = inputs_embeds[i].masked_scatter(generate_mask[i], continue_vq_features[i])
            ...
            if pixel_values is not None:
                ...
                for bs_idx, (input_embeds, input_id, pixel_value, image_grid) in enumerate(zip(inputs_embeds, input_ids, pixel_values, image_grid_thw)):
                    ...
                    pixel_value = pixel_value.type(self.visual.dtype)
                    semantic_image_embed = self.visual(pixel_value, grid_thw=image_grid)

                    mask = input_id == self.config.image_token_id

                    if pixel_vae_values is not None: # For edition consideration
                        ...
                        mask[pixel_start_index:pixel_end_index] = False

                    semantic_n_image_tokens = mask.sum().item()
                    ...
                    semantic_image_mask = mask.unsqueeze(-1).expand_as(input_embeds).to(input_embeds.device)
                    ...
                    if pixel_vae_values is not None: # For edition consideration
                        ...
                        eol_mask = (input_id ==self.config.eol_id)
                ...
                for bs_idx, (input_embeds, input_id, pixel_values_video, video_grid) in enumerate(zip(inputs_embeds, input_ids, pixel_values_videos, video_grid_thw)):
                    
                    pixel_values_video = pixel_values_video.type(self.visual.dtype)
                    video_embeds = self.visual(pixel_values_video, video_grid)
                    ...
                    video_mask = mask.unsqueeze(-1).expand_as(input_embeds).to(input_embeds.device)
        ...
        if position_ids is None and (attention_mask is None or attention_mask.ndim == 2):
            ...
            if (
                ...
                ## we only used 2d pos encode for vit feature while others only use 1d pos encoding
                ## preprocess
                # image_grid_thw = image_grid_thw.view(-1, 3) if image_grid_thw is not None else None
                image_grid_thw = torch.cat(image_grid_thw, dim=0) if image_grid_thw is not None else None
                video_grid_thw = video_grid_thw.view(-1, 3) if video_grid_thw is not None else None
                second_per_grid_ts = second_per_grid_ts.view(-1) if second_per_grid_ts is not None else None
    ...
    def prepare_mm_infer_inputs(self,
                                input_ids,
                                pixel_values,
                                pixel_vae_values,
                                image_grid_thw,
